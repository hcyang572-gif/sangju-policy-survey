...
(function () {
  ...
  /* ══════════════════════════════════════════════════════════════════
     ...
     ────────────────────────────────────────────────────────────────
     ⚠⚠ 목표 건수와 기관별 대상 인원은 «잠정치» 입니다(시안에서 가져왔습니다).
       실제 인원이 정해지면 아래 DEPTS 의 target 과 GOAL 만 고치시면
       KPI·막대·진행률·안내문이 «모두» 따라 바뀝니다. 다른 곳은 손대지 않으셔도 됩니다.
       (화면에도 「잠정」이라고 적어 두었습니다 — 확정되면 그 글자를 지우십시오)
     ══════════════════════════════════════════════════════════════════ */
...
;
  ...
  function enter(session) {
    ...
    $("dashSub").textContent =
      ...
      " · 3개 과 + 읍·면·동 행정복지센터 · 대상 " + S.GOAL + "명(잠정)"
  ...
  function paintNotice() {
    ...
    if (rows.length === 0) {
      ...
      notice("아직 들어온 응답이 없습니다",
             ...
             " " + S.총일수 + "일간이고, 대상은 " + S.GOAL + "명(잠정)입니다.\n" +
  ...
  /* ══════════════════════════════════════════════════════════════════
     [집계]
     ══════════════════════════════════════════════════════════════════ */
  // 「기관 참여율 n / m」 의 분모 — 대상 인원이 잡힌 기관만 센다(= 「그 밖의 부서」 제외).
  function 참여율모수() {
    var c = 0;
    for (var i = 0; i < S.기관.length; i++) if (S.기관[i].target > 0) c++;
    return c;
  }
  ...
  function drawDept() {
    ...
    S.기관.forEach(function (d, i) {
      var n = counts[i];
      if (n === 0 && d.target === 0) return;        // 응답도 대상도 없는 «그 밖의 부서» 는 숨긴다
